...
def get_pie_chart(entered_site,payload_slider):
    ...
    range = (spacex_df['Payload Mass (kg)']>payload_slider[0]) & (spacex_df['Payload Mass (kg)']< payload_slider[1])
    filtered_df = spacex_df[range]
    
    if entered_site == 'ALL':
        fig = px.scatter(filtered_df, x='Payload Mass (kg)',
                        y = 'class',
                        color='Booster Version Category',
                        title='Correlation Between Payloas Success for all Sites'
                        )
        return fig

    else:
        a = filtered_df[filtered_df['Launch Site']== entered_site]
        fig = px.scatter(a, x='Payload Mass (kg)',
                        y = 'class',
                        color='Booster Version Category',
                        title='Correlation Between Payloas Success for Site ')
        return fig
